...
const CodGagal = () => {
  ...
  const fetchData = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const res = await api.get('/cod-gagal', { params: { page, limit: 10, search, proses: filterProses } });
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal.mode === 'add') {
        await api.post('/cod-gagal', form);
        toast.success('Data COD Gagal berhasil ditambahkan');
      } else {
        await api.put(`/cod-gagal/${modal.data.id}`, form);
  ...
  return (
    <div className="fade-in">
      <PageHeader
        title="COD Gagal"
        subtitle="Manajemen data COD yang gagal diterima"
        actions={<ExcelImport module="cod-gagal" onSuccess={() => fetchData(1)} />}
      />
      ...
      <Modal
        ...
        title={modal.mode === 'add' ? 'Tambah COD Gagal' : 'Edit COD Gagal'}
        size="lg"
      >
