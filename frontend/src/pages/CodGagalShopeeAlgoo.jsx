...
const CodGagalShopeeAlgoo = () => {
  ...
  const fetchData = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const res = await api.get('/cod-gagal-new/shopee-algoo', { params: { page, limit: 10, search, date_from: dateFrom, date_to: dateTo } });
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal.mode === 'add') {
        await api.post('/cod-gagal-new/shopee-algoo', form);
        toast.success('Data berhasil ditambahkan');
      } else {
        await api.put(`/cod-gagal-new/shopee-algoo/${modal.data.id}`, form);
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`/cod-gagal-new/shopee-algoo/${id}`);
  ...
  return (
    <div className="fade-in">
      <PageHeader
        ...
        subtitle="Manajemen data COD gagal Shopee Algoo"
        actions={<ExcelImport module="cod-gagal-shopee-algoo" onSuccess={() => fetchData(1)} />}
      />
      <DataTable
...
export default CodGagalShopeeAlgoo;
