...
const DariCustomer = () => {
  ...
  const handleDeleteAll = async () => {
    setDeletingAll(true);
    try {
      await api.delete('/import/truncate/dari-customer');
      toast.success('Semua data reguler berhasil dihapus');
  ...
  const fetchData = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const res = await api.get('/dari-customer', {
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal.mode === 'add') {
        await api.post('/dari-customer', form);
        toast.success('Data berhasil ditambahkan');
      } else {
        await api.put(`/dari-customer/${editId}`, form);
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`/dari-customer/${id}`);
  ...
  const columns = [
    ...
    { header: 'Nama Customer', key: 'nama_customer', style: { width: '140px' } },
    { header: 'Alamat Customer', key: 'alamat_customer', style: { width: '160px' } },
  ...
  return (
    <div className="fade-in">
      <PageHeader
        title="Servis & Rusak — Service Reguler"
        subtitle="Manajemen data servis dan barang rusak dari customer reguler"
        icon="👤"
        actions={
          <button
      ...
      <Modal
        isOpen={modal.open && modal.mode === 'view'}
        onClose={() => setModal({ open: false, mode: 'add', data: null })}
        title="👁️ Detail Data Service Reguler"
        size="lg"
      >
        {modal.data && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            {[
              ...
              ['Nama Customer', modal.data.nama_customer],
              ['Alamat Customer', modal.data.alamat_customer],
      ...
      <Modal isOpen={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="🗑️ Konfirmasi Hapus" size="sm">
        <p style={{ color: '#64748b', marginBottom: '20px' }}>
          Apakah Anda yakin ingin menghapus data <strong>{deleteConfirm?.nama_customer}</strong>? Tindakan ini tidak dapat dibatalkan.
        </p>
      ...
      {/* Delete All Confirm */}
      <Modal isOpen={deleteAllConfirm} onClose={() => setDeleteAllConfirm(false)} title="🗑️ Konfirmasi Hapus Semua" size="sm">
        <p style={{ color: '#64748b', marginBottom: '20px' }}>
          Apakah Anda yakin ingin menghapus <strong>seluruh data</strong> reguler? Tindakan ini tidak dapat dibatalkan.
        </p>
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
