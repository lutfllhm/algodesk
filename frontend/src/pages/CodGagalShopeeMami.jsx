...
const EMPTY_FORM = {
  ...
  status_pembatalan_pengembalian: '', status_pengiriman_gagal: '', no_resi: '',
...
const CodGagalShopeeMami = () => {
  ...
  const fetchData = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const res = await api.get('/cod-gagal-new/shopee-mami', { params: { page, limit: 10, search, date_from: dateFrom, date_to: dateTo } });
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal.mode === 'add') {
        await api.post('/cod-gagal-new/shopee-mami', form);
        toast.success('Data berhasil ditambahkan');
      } else {
        await api.put(`/cod-gagal-new/shopee-mami/${modal.data.id}`, form);
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`/cod-gagal-new/shopee-mami/${id}`);
  ...
  const columns = [
    ...
    { header: 'Opsi Pengiriman', key: 'opsi_pengiriman', style: { width: '130px' } },
    { header: 'Antar ke Counter', key: 'antar_ke_counter', style: { width: '130px' } },
    ...
    { header: 'Waktu Pesanan Dibuat', key: 'waktu_pesanan_dibuat', style: { width: '160px' } },
    { header: 'Waktu Pembayaran', key: 'waktu_pembayaran_dilakukan', style: { width: '150px' } },
    { header: 'Metode Pembayaran', key: 'metode_pembayaran', style: { width: '140px' } },
    { header: 'Status Klaim', key: 'status_klaim', style: { width: '110px' } },
    { header: 'Tgl Klaim Diajukan', key: 'tanggal_klaim_diajukan', style: { width: '140px' } },
    { header: 'Tgl Klaim Disetujui', key: 'tanggal_klaim_disetujui', style: { width: '140px' } },
    { header: 'Tgl Klaim Dicairkan', key: 'tanggal_klaim_dicairkan', style: { width: '140px' } },
    { header: 'Tgl Klaim Ditolak', key: 'tanggal_klaim_ditolak', style: { width: '130px' } },
    { header: 'Jumlah Kompensasi', render: r => r.jumlah_kompensasi ? Number(r.jumlah_kompensasi).toLocaleString('id-ID') : '-', style: { width: '140px' } },
    {
  ...
  return (
    <div className="fade-in">
      <PageHeader
        title="COD Gagal Shopee Mami Kasir"
        subtitle="Manajemen data COD gagal Shopee Mami Kasir"
        actions={<ExcelImport module="cod-gagal-shopee-mami" onSuccess={() => fetchData(1)} />}
      />
      <DataTable
      ...
      <Modal isOpen={modal.open} onClose={() => setModal({ open: false, mode: 'add', data: null })}
        title={modal.mode === 'add' ? 'Tambah COD Gagal Shopee Mami Kasir' : 'Edit COD Gagal Shopee Mami Kasir'} size="lg">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', maxHeight: '65vh', overflowY: 'auto', padding: '4px' }}>
          ...
          <Field label="Opsi Pengiriman" fieldKey="opsi_pengiriman" />
          <Field label="Antar ke Counter/Pick-up" fieldKey="antar_ke_counter" />
          <Field label="Pesanan Harus Dikirim Sebelum" fieldKey="pesanan_harus_dikirim_sebelum" type="datetime-local" />
          <Field label="Waktu Pengiriman Diatur" fieldKey="waktu_pengiriman_diatur" type="datetime-local" />
          ...
          <Field label="Nama Variasi" fieldKey="nama_variasi" />
          ...
          <Field label="Waktu Pesanan Dibuat" fieldKey="waktu_pesanan_dibuat" type="datetime-local" />
          <Field label="Waktu Pembayaran Dilakukan" fieldKey="waktu_pembayaran_dilakukan" type="datetime-local" />
          <Field label="Metode Pembayaran" fieldKey="metode_pembayaran" />
          <Field label="Status Klaim" fieldKey="status_klaim" />
          <Field label="Tanggal Klaim Diajukan" fieldKey="tanggal_klaim_diajukan" type="date" />
          <Field label="Tanggal Klaim Disetujui" fieldKey="tanggal_klaim_disetujui" type="date" />
          <Field label="Tanggal Klaim Dicairkan" fieldKey="tanggal_klaim_dicairkan" type="date" />
          <Field label="Tanggal Klaim Ditolak" fieldKey="tanggal_klaim_ditolak" type="date" />
          <Field label="Jumlah Kompensasi" fieldKey="jumlah_kompensasi" type="number" />
        </div>
...
export default CodGagalShopeeMami;
