...
/** Nilai untuk input type="date" (YYYY-MM-DD) dari API / MySQL */
...
const ProsesBadge = ({ proses }) => {
  const map = {
    'Banding': { bg: '#fef3c7', color: '#d97706' },
    'Selesai': { bg: '#d1fae5', color: '#059669' },
    'Tidak Banding': { bg: '#fee2e2', color: '#dc2626' }
  };
...
const ReturTiktok = () => {
  ...
  const fetchData = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const res = await api.get('/retur/tiktok', { params: { page, limit: 10, search, proses: filterProses, date_from: dateFrom, date_to: dateTo } });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (modal.mode === 'add') { await api.post('/retur/tiktok', payload); toast.success('Retur TikTok berhasil ditambahkan'); }
      else { await api.put(`/retur/tiktok/${editId}`, payload); toast.success('Retur TikTok berhasil diperbarui'); }
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`/retur/tiktok/${id}`);
  ...
  return (
    <div className="fade-in">
      <PageHeader title="Retur TikTok" subtitle="Manajemen retur dan banding TikTok" icon="🎵"
        actions={<ExcelImport module="retur-tiktok" onSuccess={() => fetchData(1)} />}
      />
      ...
      <Modal isOpen={modal.open} onClose={() => setModal({ open: false, mode: 'add', data: null })}
        title={modal.mode === 'add' ? '➕ Tambah Retur TikTok' : '✏️ Edit Retur TikTok'} size="lg">
